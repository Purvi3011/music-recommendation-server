...
const musicSchema = new mongoose.Schema({
    ...
    valence: Number,
    play_count: Number,
    percentageHeard: Number, // New field to store the percentage of the song heard
    repeatRate: Number // New field to store the repeat rate of the song
})
...
app.post('/addMusicToListeningHistory', async (req, res) => {
    const currTrackId = req.body.currTrackId;
    const currUserId = req.body.currUserId;
    const listenTime = new Date(); // Get the current time

    try {
        const user = await User.findById(currUserId);
        let listeningActivity = user.listeningActivity;
        if (!listeningActivity) {
            listeningActivity = [{ trackId: currTrackId, listenTime: listenTime }];
        } else {
            if (listeningActivity.length >= 5) {
                listeningActivity.shift(); // remove oldest element from front of array
            }
            listeningActivity.push({ trackId: currTrackId, listenTime: listenTime });
        }
        user.listeningActivity = listeningActivity;

        // Calculate the percentage of the song heard
        const music = await Music.findOne({ track_id: currTrackId });
        const songDuration = music.duration; // Duration of the song in milliseconds
        const currentTime = listenTime.getTime(); // Current time in milliseconds
        const startTime = currentTime - songDuration; // Start time of the song in milliseconds
        const percentageHeard = ((currentTime - startTime) / songDuration) * 100;

        // Store the percentage of the song heard in the database
        music.percentageHeard = percentageHeard;

        // Calculate the repeat rate of the song
        const repeatRate = listeningActivity.filter(item => item.trackId === currTrackId).length;

        // Store the repeat rate of the song in the database
        music.repeatRate = repeatRate;
